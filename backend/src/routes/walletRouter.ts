...
router.get("/balance", authenticate, async (req: CustomRequest, res: Response) => {
...
router.post("/withdraw", authenticate, async (req: CustomRequest, res: Response) => {
  const userId = req.user?.id;
...
router.post('/webhook', bodyParser.raw({ type: 'application/json' }), async (req: Request, res: Response) => {
  ...
  try {
    const event = stripe.webhooks.constructEvent(payload, sig, process.env.ENDPOINT_SECRET!);

    switch (event.type) {
      case 'payment_intent.succeeded':
        const paymentSucceededIntent = event.data.object as Stripe.PaymentIntent;

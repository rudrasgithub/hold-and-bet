...
import { cardRecord, generateRandomCards, validHoldValues } from "../config/lib";

const router = Router();

router.post("/newgame", authenticate, async (req: CustomRequest, res: Response) => {
  ...
  try {
    ...
    if (!wallet || wallet.balance < 5) {
      ...
      res.status(400).json({ error: "Insufficient balance to start a new game. Minimum required: 5" });
      return;
    }
...
router.post('/:gameId/hold', authenticate, async (req: Request, res: Response) => {
  const gameId = req.params.gameId;
  const { hold } = req.body;
...
router.post("/:gameId/bet", authenticate, async (req: CustomRequest, res: Response) => {
  const gameId = req.params.gameId;
  console.log(gameId);
  const { bets } = req.body;
  console.log(bets)
  ...
  try {
    let gameState = JSON.parse((await redisClient.get(`game:${gameId}`)) || "{}");
    ...
    for (const [card, amount] of Object.entries(bets)) {
      ...
      if (card === holdedCard) {
        res.status(400).json({ error: `Cannot place a bet on the held card: ${holdedCard}` });
        return;
      }
      if (typeof amount !== "number" || amount <= 0) {
        res.status(400).json({ error: `Invalid bet amount for card ${card}` });
        return;
      }
...
router.post("/:gameId/reveal", authenticate, async (req: CustomRequest, res: Response) => {
  ...
  try {
    ...
    const cardResults: { [key: string]: { bet: number, loss?: number, gain?: number} } = {};
    ...
    await prisma.$transaction([
      ...
      prisma.game.update({
        where: { id: gameId },
        data: { status: "Completed", outcome: totalEarnings > 0 ? "Won" : "Lost" },
      }),
    ]);
